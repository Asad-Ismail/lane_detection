...
out_video = cv2.VideoWriter('drive_cut_pred_tflite_quant_activ.mp4',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (640,384))
...
def main():
    ...
    lite_model=pred_lite("cluster_quantized_activations.tflite")
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            rgb_lite[...,1]*=3
            rgb_lite[...,2]*=2
            cv2.putText(rgb_lite, 'TF Lite int8 weights', (300,30), cv2.FONT_HERSHEY_PLAIN, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
            #tf lanes
            #rgb_tf=cv2.cvtColor(tf_lanes, cv2.COLOR_GRAY2BGR)
            #rgb_tf[...,0]*=2
            #rgb_tf[...,1]*=3
            #rgb_tf[...,2]*=2
            #cv2.putText(rgb_tf, 'TF Float32 Model', (300,30), cv2.FONT_HERSHEY_PLAIN, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
